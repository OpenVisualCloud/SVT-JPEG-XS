...
colour_format | Specifies the format of input video,   please refer to ColourFormat_t enum | mandatory | N/A | Tested: (COLOUR_FORMAT_PLANAR_YUV420, COLOUR_FORMAT_PLANAR_YUV422, COLOUR_FORMAT_PLANAR_YUV444_OR_RGB, COLOUR_FORMAT_PACKED_YUV444_OR_RGB, COLOUR_FORMAT_PLANAR_4_COMPONENTS (4:4:4:4, e.g. RGBA/YUVA444), COLOUR_FORMAT_PLANAR_YUV422_ALPHA (4:2:2:4, YUV422+alpha)),   experimental: (COLOUR_FORMAT_YUV400). Note: COLOUR_FORMAT_PACKED_YUV444_OR_RGB is not supported with cpu_profile=1 (CPU_PROFILE_CPU), only cpu_profile=0 (Low latency)
...
enable_color_transform | Coding feature: encoder-side reversible colour transform (RCT, ISO/IEC 21122-1 Table F.2, signalled as Cpih=1) applied before the wavelet transform. Requires colour_format == COLOUR_FORMAT_PLANAR_YUV444_OR_RGB (exactly 3, unsubsampled planar components) and cpu_profile == 0 (Low latency); rejected otherwise at svt_jpeg_xs_encoder_init() time. Recommended for RGB-like input only (measured +5.9dB PSNR at matched bpp on real footage) - accepted for yuv444 input too (same format check) but not recommended there (measured -2.9dB PSNR: the transform decorrelates RGB-like input into YCbCr-like, which does not help input that is already YCbCr-like) | optional | 0 (disable) | 0(disable), 1(enable)
