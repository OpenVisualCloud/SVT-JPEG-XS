...
MATRIX=(
    ...
    # unless the input pixel format is planar RGB or planar YUV444. Baselines reuse the
    # 1080p60_yuva444p8 row above (same resolution/bit-depth, no dedicated yuv444+RCT baseline
    # measured yet) - RCT is a fixed per-pixel matrix multiply layered on the same DWT/GC
    # pipeline, so it is not expected to shift throughput materially either direction.
