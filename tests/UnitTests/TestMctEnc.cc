 ...
 *
 * Run once per asm tier: inverse_rct() is CPU-dispatched (C/AVX2/AVX512 kernels), and this is
 * the one place that checks it against forward_rct_line() bit-exactly rather than just against
 * itself, so it is the test that would actually catch a wrong AVX2/AVX512 kernel. */
class MctEnc : public ::testing::TestWithParam<CPU_FLAGS> {};

TEST_P(MctEnc, ForwardRctIsExactInverseOfDecoderInverseRct) {
    setup_decoder_rtcd_internal(GetParam());
...
INSTANTIATE_TEST_SUITE_P(C_AVX2_AVX512, MctEnc, ::testing::Values(0, CPU_FLAGS_AVX2, CPU_FLAGS_ALL));
