#!/bin/bash
# Local runner for .github/workflows/base_build.yaml and linter.yaml (Linux jobs only)
# Windows jobs are skipped.
#
# Usage: ./run_ci_locally.sh [OPTIONS]
#   --skip-deps          Skip apt-get dependency installation
#   --skip-linter        Skip super-linter (requires Docker)
...
do_lint() {
    if ! command -v docker &>/dev/null; then
        echo "ERROR: docker is required for super-linter but was not found in PATH."
        return 1
    fi
    docker run --rm \
        -e RUN_LOCAL=true \
        -e USE_FIND_ALGORITHM=true \
        -e FILTER_REGEX_EXCLUDE='.*/(Build|Bin|third_party)/.*' \
        -e DISABLE_ERRORS=false \
        -e VALIDATE_ALL_CODEBASE=true \
        -e BASH_FILE_NAME=.shellcheck \
        -e BASH_SEVERITY=error \
        -e LINTER_RULES_PATH=.github/config \
        -e GITHUB_COMMENT_ON_PR=false \
        -e GITHUB_STATUS_UPDATES=false \
...
# ── Main ──────────────────────────────────────────────────────────────────────
echo ""
echo "Local CI Runner — SVT-JPEG-XS base_build.yaml + linter.yaml (Linux)"
echo "  Repo:        $REPO_ROOT"
...
if $SKIP_LINTER; then
    skip_step "super-linter"
else
    run_step "super-linter" do_lint
fi
