...
/* The parts of the group parser that carry no instruction set of their own:
 * the short reader, the nibble loads and the sign table. Every vector
 * implementation of unpack_data builds on these, so they live outside the
 * per-ISA directories. */

#ifndef __UNPACK_SHARED_H__
#define __UNPACK_SHARED_H__
